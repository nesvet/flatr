...
$temporaryDirectory = New-Item -ItemType Directory -Path (Join-Path $env:TEMP ("fln-" + [Guid]::NewGuid().ToString("n")))
try {
	...
	Write-Output "Installed fln to $destinationPath"

	if (Get-Command fln -ErrorAction SilentlyContinue) {
		Write-Output "Run: fln --help"
	} else {
		Write-Output "Add $installDirectory to your PATH to use fln."
	}
} finally {
